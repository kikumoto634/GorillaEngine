...
ShadowMap *ShadowMap::Create(UINT texNumber, Vector2 pos, Vector2 size, XMFLOAT4 color, Vector2 anchorpoint, bool isFlipX, bool isFlipY)
{
	// Spriteのインスタンスを生成
	ShadowMap* shadowMap =
		new ShadowMap(texNumber, {pos.x,pos.y,0.f}, size, color, anchorpoint, isFlipX, isFlipY);
...
ShadowMap::ShadowMap(UINT texnumber, Vector3 pos, Vector2 size, XMFLOAT4 color, Vector2 anchorpoint, bool isFlipX, bool isFlipY)
	: Sprite(
		texnumber,
		pos,
		size,
...
bool ShadowMap::Initialize()
{
	...
	HRESULT result;
	{
		...
		CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Buffer((sizeof(ConstShadowMap)+0xff)&~0xff);
	...
	lightCamera->Initialize({0,600,0}, {1.5f,0,0});
...
void ShadowMap::Draw()
{
	...
	HRESULT result{};
	{
		ConstShadowMap* constMap = nullptr;
		result = constBuff->Map(0,nullptr,(void**)&constMap);
		assert(SUCCEEDED(result));
		constMap->world = matWorld;
		constMap->view = lightCamera->GetMatView();
		constMap->projection = lightCamera->GetMatProjection();
...
void ShadowMap::TextureInitialize()
{
	HRESULT result;
	//テクスチャリソース
	{
...
void ShadowMap::DepthInitialize()
{
	HRESULT result;
	//深度バッファ
	{
